...
public:
    ...
    StatusType AddTask(int taskID, int priority){
        try {
            ...
            HeapTask& heapR = MaxPriHeap.getElement(avlP->GetMaxIndex());
            avlP->SetMaxTask(&heapR);
            HeapTask& heapRR=MinPriHeap.getElement(avlP->GetMinIndex());
            avlP->SetMinTask(&heapRR);
        } catch (bad_alloc& b) {
